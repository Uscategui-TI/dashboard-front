...
export const Metrics2 = () => {
  ...
  const [prevRechazados, setPrevRechazados] = useState(0);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${apiUrl}/api/prospecto/estadisticas`);
        const rechazadosCount = response.data.conteoPorEstado.RECHAZADA || 0; // ✅ Solo RECHAZADA
        setRechazados(rechazadosCount);
        setPrevRechazados(rechazadosCount - 1); // simula valor anterior
      } catch (error) {
  ...
  const cambioRechazados = calcCambio(prevRechazados, rechazados);
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03] w-full sm:max-w-[380px] flex flex-col justify-between h-[360px]">
      <div className="flex flex-col justify-between h-full">
        ...
        <div className="flex justify-center">
          <Badge color={cambioRechazados >= 0 ? "success" : "error"}>
            {cambioRechazados >= 0 ? <ArrowUpIcon /> : <ArrowDownIcon />}
            {Math.abs(cambioRechazados)}%
          </Badge>
        </div>
      </div>
    </div>
